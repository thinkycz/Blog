...
class CommentRepositoryTest extends ApplicationTestCase
{
	protected function _getRepo() {
		return $this->container->get('cvut_fit_biwt1_blog_common.repository.comment');
	}
	...
	protected function _createPost() {
		$repo = $this->container->get('cvut_fit_biwt1_blog_common.repository.post');
		$now = new \DateTime;
	...
	protected function _create() {
		...
		$object = new Comment;
		$object->setAuthor($this->_createUser());
		$object->setPost($this->_createPost());
		$object->setParent(NULL);
		$object->setText('Text 1');
		$object->setCreated($now);
		$object->setModified($now);
		$object->setSpam(FALSE);
